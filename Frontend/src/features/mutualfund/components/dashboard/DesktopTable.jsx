        ...
        <TableBody>
          {portfolio?.map((item) => (
            <TableRow onClick={() => navigate(`/mutual-funds/${item.schemeCode}`)} key={item.schemeCode}>
              ...
              <TableCell className="text-base font-medium">₹{item.invested} </TableCell>

              <TableCell className={`${item.current >= item.invested ? "text-green-500" : "text-red-500"} text-base`}>
                ₹{item.current}
              </TableCell>
              ...
              <TableCell className="text-base">
                <div className={`${item.pnl >= 0 ? "text-green-500" : "text-red-500"} space-x-4 text-start`}>
                  <p>₹{item.pnl}</p>
                  <p className={`${item.returnPercent >= 0 ? "text-green-500" : "text-red-500"} text-sm font-light`}>
                    ({item.returnPercent}%)
                  </p>
                </div>
              </TableCell>
