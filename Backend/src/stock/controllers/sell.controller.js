...
export const sellSomeQty = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({ success: true, message: `${quantity} quantity sold successful` });
});
...
export const sellAllQty = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({ success: true, message: "All quantity sold successfully" });
});
